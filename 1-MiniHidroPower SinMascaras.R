...
# First Version December 20 2018
...
#II. Raster y Vectoriales
{
  ...
  # Direccones Archivos
  CharFile1 <- "RasterFiles/A-RasterRio.TIF"
  CharFile2 <- "RasterFiles/B-RasterDEM.TIF"
...
#III. Malla de puntos
{
  ...
  RasMHP <- RasDEM                                                                                           #1
  RasMHP$PotenciakW <- NA;   RasMHP$GastoMax <- NA;   RasMHP$HMax     <- NA                                  #4
  RasMHP$xTurbina   <- NA;   RasMHP$yTurbina <- NA;   RasMHP$zTurbina <- NA;   RasMHP$BaseTurbina <- NA      #8
  RasMHP$xIntake    <- NA;   RasMHP$yIntake  <- NA;   RasMHP$zIntake  <- NA;   RasMHP$BaseIntake  <- NA      #12
...
fNumCruces <- function(NumCoordsRioRadiox,NumCoordsRioRadioy,NumCoordPiletaix,NumCoordPiletaiy) {
  ...
  Numxc <- c(NumCoordsRioRadiox, NumCoordPiletaix); Numyc <- c(NumCoordsRioRadioy, NumCoordPiletaiy)
 ...
 for(i in c(1,50000,100000,250000,350000,450000,550000,600000,700000,850000,900000,
  ...
  ### PILETA ###
  {
    #1. Inicio: Ubicar la pileta
    {
      ...
      #i=450000+350;# 950001
      #i=916800 # Sobre NA's 
      #i=916800+12 # Bloqueado por NA's
    ...
    #2. Si rio esta fuera del radio de la Pileta (NEXT)
    {
      ...
      if(length(SPPointsRioRadio)==0){
        SPPointsGrid$PotenciakW[i] <- 0; SPPointsGrid$GastoMax[i] <- 0; SPPointsGrid$HMax[i] <- 0
    ...
    #3. Trayectorias Rectas (Pileta-Rio) que solamente toquen al rio 1 vez (NEXT)
    {
      ...
      NumCoordsRioRadiox <- SPPointsRioRadio@coords[,1]; NumCoordsRioRadioy <- SPPointsRioRadio@coords[,2] # Son todos los puntos de la interseccion
      NumCoordPiletaix <- SPPointPiletai@coords[1]; NumCoordPiletaiy <- SPPointPiletai@coords[2]   # Es el punto actual sobre el DEM
  ...
  ### TURBINA ###
  {
    ...
    SPPointTurb <- SPPointsTurbina[k3[1],] # Posicion con menor altura sin obstaculos
